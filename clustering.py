...
if(graph==1):
    ...
    for i in range(0, len(time), s*45):
        ...
        if(alt):
            ...
            plt.text(time[i], 0.1, kmeans.labels_[a], fontsize=10, color='red')
            alt = not alt
    plt.ylabel('Accelerometer RMS (g)')
    plt.title('Accelerometer RMS overlayed with movement and cluster')
    plt.xlabel('Time (ms)')
    plt.legend(loc='best')
    plt.grid(True)
    plt.show()